...
def log_data(self,order):
    ...
    if order.status in [order.Completed]:
        output += ( f"Cost: {order.executed.value:6.2f} " +
                    ...
                    #f"SMA200: {float(self.SMA200[0]):3.2f} ")
                    )
        if not order.isbuy():
    ...
    def next(self):
        ...
        if not self.position:
            if buy_order_finished == 1:
                ...
                if self.SMA400 > self.SMA600:
                    if self.SMA25[-1] > self.SMA50[-1]:# and self.SMA25[0] > self.SMA50[0]:
                        if self.SMA50[-1] < self.SMA100[-1] and self.SMA50[0] > self.SMA100[0]:
                            ...
                            if self.rsi6 < 65: #and self.rsi[-1] < 15:# and self.rsi[0] < 25:
                                ...
                                #print(f'order finished? {buy_order_finished}')
                                self.buy(exectype=bt.Order.Limit,price=buyprice,size=buysize,valid=bt.date2num(data.num2date())+(5/1440))
                                buy_order_finished = 0
        
            #print(f'MACD: {abs((float(self.macd[0])-float(self.macd[-1]))/2)}')
            #print(f'lot size: {buysize}, lot price: {buyprice}')
        ...
        if self.position.size > 0:
            if sell_order_finished == 1:
                ...
                #if self.SMA25[-1] > self.SMA50[-1] and self.SMA25[0] < self.SMA50[0]:
                if data.close[-1] > self.SMA50[-1] and last_closing_price < self.SMA50[0]:
                    #if self.rsi > 90: 
